...
// function decodePRSTFile(bytes: Uint8Array) {
export function decodePRSTFile(buffer: Buffer): IPresetInfo {
...
// function decodePresetData(buffer: Buffer) {
    ...
    for (let i = 0; i < 11; i=i+1) {
        ...
        const effectInfo = decodeEffectModule(buffer, offset);
        offset += 72
...
function decodeEffectModule(buffer: Buffer, offset: number): IEffectInfo {
    ...
    for (let i = 0 ; i < 15; i=i+1) {
        ...
        const param = uint8BytesToFloat32(paramEncoded);
...
function decodeExpSettings(buffer: Buffer, offset: number): IExpSettings {
    ...
    const paramMax = uint8BytesToFloat32([...buffer.subarray(offset, offset + 4)]);
    offset += 4;  
    const paramMin = uint8BytesToFloat32([...buffer.subarray(offset, offset + 4)]);
...
function decodeKnobSettings(buffer: Buffer, offset: number): IKnobSettings {
...
function decodeCtrlSettings(buffer: Buffer, offset: number): ICtrlSettings {
...
function uint8BytesToFloat32(bytes: number[]): number {
