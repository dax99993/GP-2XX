...
export class DeserializeEffect {
  deserialize(ieffect: IDefaultEffect): EffectModel {
    ...
    params = ieffect.params.map((p) =>
      deserializeParam.deserialize(p),
    ) as IParameter[];
    ...
    const e = new EffectModel(
      ieffect.name,
      ieffect.ID,
      "",
      EffectType[effectModule],
      true,
      params,
    );
...
export class EffectModel {
  ...
  setParameterValue(parameterID: number, value: number) {
    ...
    if (params.length == 0) {
      throw new Error(
        `There is no parameter in effect ${this.name} with ID ${parameterID}`,
      );
      ...
      //   console.log("active bind params", this.activeBindParams);
      //   console.log("Combox index", combox_index);

      if (combox_index === -1) {
        throw new Error(
          `There should be one Combox parameter if there are 2 parameters in effect ${this.name} with same ID ${parameterID}`,
        );
      }
      ...
      throw new Error(
        `There should not be more than 2 parameters in effect ${this.name} with ID ${parameterID}`,
      );
    }
    ...
    if (p instanceof Switch) {
      ...
      if (bind_index != null && value !== 0) {
        ...
        // Reset param
        const q = this.parameters.find(
          (p) => p.ID === bind_index && p.type === ParamType.Combox,
        );
        q?.reset();
      } else if (bind_index != null && value == 0) {
        // Remove bind from active bind
        this.activeBindParams = this.activeBindParams.filter(
          (bind) => bind !== bind_index,
        );

        // Reset param
        const q = this.parameters.find(
          (p) => p.ID === bind_index && p.type === ParamType.Knob,
        );
        q?.reset();
      }
    }
  ...
  static fromEffectInfo(effectInfo: IEffectInfo): EffectModel {
    ...
    const isParamDecimal =
      effectInfo.chainID === EffectType.MOD ||
      effectInfo.chainID == EffectType.DLY;
    ...
    for (let i = 0; i < e.parameters.length; i = i + 1) {
      ...
      console.log(
        `Setting parameter ${ID} to value ${effectInfo.paramValues[ID]}`,
      );
    }
  ...
  static defaultFromID(effectID: number, effectType: EffectType): EffectModel {
    ...
    const effectsInfo: IDefaultEffect[] = EffectsCatalog.filter(
      (c) => c.name === key,
    )[0].effects;
    ...
    throw new Error(
      `Effect not found!, check correct ID ${effectID} - ${effectType}`,
    );
  }
}
