...
export class PresetImporter {
    ...
    async LoadFiles(): Promise<Boolean> {
        ...
        if (!documents.canceled) {
            ...
            for (let i = 0; i < documents.assets.length; i = i + 1) {
                ...
                // const presetInfo = await this.decodePRSTBinaryFile(buffer);
    ...
    decodeFiles(): IPresetInfo[] {
        ...
        this.binaryFiles.forEach(buffer => {
            const presetInfo = this.decodePRSTBinaryFile(buffer);
            this.presets.push(presetInfo);
        });
    ...
    decodePRSTBinaryFile(buffer: Buffer): IPresetInfo {
    ...
    decodeEffectModule(buffer: Buffer, offset: number): IEffectInfo {
        ...
        for (let i = 0; i < 15; i = i + 1) {
            ...
            const param = this.uint8BytesToFloat32(paramEncoded);
    ...
    decodeExpSettings(buffer: Buffer, offset: number): IExpSettings {
        ...
        const paramMax = this.uint8BytesToFloat32([...buffer.subarray(offset, offset + 4)]);
        offset += 4;
        const paramMin = this.uint8BytesToFloat32([...buffer.subarray(offset, offset + 4)]);
