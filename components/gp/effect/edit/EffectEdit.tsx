        ...
        {store.gp200.currentEffect &&
          store.gp200.currentEffect.parameters.map((p) => {
            if (
              ...
              !store.gp200.currentEffect?.activeBindParams.includes(p.index)
            ) {
              return <NumericParameter key={p.name} param={p} />;
              ...
              if (
                ...
                return <SelectParameter key={p.name} param={p as Combox} />;
              }
            } else if (p.type === ParamType.Switch) {
              return <SwitchParameter key={p.name} param={p as SwitchParam} />;
            }
          })}
      </ScrollView>
    </VStack>
