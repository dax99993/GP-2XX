import { MIDIAccess, requestMIDIAccess } from "@motiz88/react-native-midi";
...
function useMIDIAccess(sysex: boolean = false) {
    ...
    useEffect(() => {
        async function getMidi() {
            const m = await requestMIDIAccess({ sysex });
            setMidi(m);
        }
...
export function useMidiIoSetup() {
  ...
  const handleMidiStateChange = useCallback(() => {
    console.log(midiAccess?.inputs.size, midiAccess?.outputs.size);
    ...
    if (outputPort == null && midiAccess && midiAccess.outputs.size) {
        console.log(midiAccess.outputs.size);
